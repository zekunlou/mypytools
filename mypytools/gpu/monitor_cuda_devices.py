...
def monitor_gpus(interval: int = 10, duration: int = 24 * 3600, log_fpath="gpu_usage.log"):
...
def visualize_gpus_usage(log_fpath: str, ax=None):
    ...
    for color_idx, (key, grp) in enumerate(df.groupby("gpu_id")):
        ax[1].plot(grp.index, grp["memory_used"], color=f"C{color_idx}", label=f"GPU {key}")
        ax[1].axhline(grp["memory_total"][0], min(grp.index), max(grp.index), linestyle="dotted", color=f"C{color_idx}")
